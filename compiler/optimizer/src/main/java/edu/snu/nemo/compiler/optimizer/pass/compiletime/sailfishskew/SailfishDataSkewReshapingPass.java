...
public final class SailfishDataSkewReshapingPass extends ReshapingPass {
  ...
  public DAG<IRVertex, IREdge> apply(final DAG<IRVertex, IREdge> dag) {
    ...
    dag.topologicalDo(v -> {
      ...
      if (v instanceof OperatorVertex && dag.getOutgoingEdgesOf(v).stream().anyMatch(irEdge ->
        ...
        dag.getOutgoingEdgesOf(v).forEach(edge -> {
          ...
          if (DataCommunicationPatternProperty.Value.Shuffle
            ...
            final IREdge newEdge = new IREdge(DataCommunicationPatternProperty.Value.OneToOne,
                v, metricCollectionBarrierVertex);
            ...
            final IREdge edgeToGbK = new IREdge(DataCommunicationPatternProperty.Value.Shuffle,
                metricCollectionBarrierVertex, edge.getDst(), edge.isSideInput());
        ...
        builder.addVertex(v);
        dag.getOutgoingEdgesOf(v).forEach(builder::connectVertices);
      }
    });
